...
{
    $pdo = PDOConnector::connect($_GET['name']);
    ...
    {
        $tableName = $table['Tables_in_' . $_GET['name']];

        $result .= '<tr>';
        $result .= '<td>'.$tableName.'</td>';
        $result .= '<td><a class="w3-container w3-button w3-blue" href="table.php?database=' . $_GET['name'] . '&name=' . $tableName . '">View</a></td>';
        $result .= '</td>';
    }
...
<html>
<head>
    <title>&lambda; Databases Manager - <?php echo $_GET['name']; ?> database</title>
    <meta charset="utf-8">
